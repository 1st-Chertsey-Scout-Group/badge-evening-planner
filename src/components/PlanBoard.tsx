...
import { clearPlan, getPlan, onPlanChange, removeFromPlan } from '@/lib/plan'
import { getAllTicked, onProgressChange } from '@/lib/storage'
import { Clock, Trash2, X } from 'lucide-preact'
...
export default function PlanBoard({ bases, badges }: Props) {
  ...
  useEffect(() => {
    ...
    syncTicks()
    setLoaded(true)
    return combine(onPlanChange(syncPlan), onProgressChange(syncTicks))
  }, [])
  ...
  if (loaded && selected.length === 0) {
    return (
      <div class="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center">
        ...
        <p class="mt-1 text-sm text-slate-500">Add bases to plan an evening.</p>
        <a
          href="/bases"
    ...
    <div class="space-y-6">
      ...
      <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-4 lg:col-span-2">
          <h2 class="text-sm font-semibold tracking-wide text-slate-500 uppercase">Badge coverage</h2>
  ...
  return (
    ...
    >
      <div class="flex items-center gap-3 p-4">
        <img src={badge.img.src} alt={badge.img.alt} width={40} height={40}
          class="h-10 w-10 shrink-0 rounded-lg bg-white object-contain" />
        <div class="min-w-0 flex-1">
          <a href={`/badges/${badge.slug}`} class="font-semibold text-slate-900 hover:text-scout-purple">
            {badge.title}
          </a>
      ...
      <div class="h-1.5 w-full bg-slate-100">
        <div class="h-full bg-scout-purple/30" style={`width:${p.projected.percent}%`}>
          <div class="h-full bg-scout-purple" style={`width:${pctRatio(p.current.percent, p.projected.percent)}%`} />
        </div>
      </div>
      ...
      <ul class="mt-3 space-y-2">
        {bases.map((b) => (
          <li key={b.slug} class="flex items-center gap-2">
            <a href={`/bases/${b.slug}`} class="flex-1 text-sm text-slate-700 hover:text-scout-purple">
              {b.title}
            </a>
